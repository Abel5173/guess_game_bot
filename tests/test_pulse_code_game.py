...
class TestPulseCodeGame(unittest.TestCase):
    ...
    def test_make_guess(self):
        ...
        hits, flashes, static = self.game.make_guess(player_id=123, target_id="AI-Calculon", guess="1234")
        self.assertEqual(hits, 4)
        self.assertEqual(flashes, 0)
        self.assertEqual(static, 0)

        # Test a guess with some hits and flashes
        hits, flashes, static = self.game.make_guess(player_id=123, target_id="AI-Calculon", guess="1325")
        self.assertEqual(hits, 1)
        self.assertEqual(flashes, 2)
        self.assertEqual(static, 1)

        # Test a guess with only flashes
        hits, flashes, static = self.game.make_guess(player_id=123, target_id="AI-Calculon", guess="4321")
        self.assertEqual(hits, 0)
        self.assertEqual(flashes, 4)
        self.assertEqual(static, 0)

        # Test a guess with only static
        hits, flashes, static = self.game.make_guess(player_id=123, target_id="AI-Calculon", guess="5678")
        self.assertEqual(hits, 0)
        self.assertEqual(flashes, 0)
