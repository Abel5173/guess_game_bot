    ...
    for model in models:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                print(
                    f"🤖 AI Response: {result[0]['generated_text'] if result else 'No response'}"
                )
                return True, model
...
async def test_bot_tasks():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(
                f"🤖 AI Riddle: {result[0]['generated_text'] if result else 'No response'}"
            )
        elif response.status_code == 503:
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(
                f"🤖 AI Clue: {result[0]['generated_text'] if result else 'No response'}"
            )
        elif response.status_code == 503:
            print("⚠️ Model is still loading, this is normal")
        else:
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(
                f"🤖 AI Complex Clue: {result[0]['generated_text'] if result else 'No response'}"
            )
        elif response.status_code == 503:
            print("⚠️ Model is still loading, this is normal")
        else:
    ...
    while True:
        try:
            ...
            if response.status_code == 200:
                result = response.json()
                ai_response = (
                    result[0]["generated_text"] if result else "I have no response."
                )
                print(f"🤖 AI: {ai_response}")
            elif response.status_code == 503:
