...
async def show_basecamp_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    user = update.effective_user
    basecamp_display = engagement_engine.basecamp_system.generate_basecamp_display(user.id)
...
async def show_wager_options_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
...
async def place_wager_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        success, message = engagement_engine.risk_reward_system.place_wager(user.id, wager_type)
...
async def claim_mission_rewards_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for mission_id, progress in missions.items():
        if progress["completed"] and not progress.get("claimed", False):
            success, message = engagement_engine.mission_system.claim_mission_rewards(user.id, mission_id)
            if success:
                claimed_count += 1
...
async def show_cards_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    user = update.effective_user
    card_display = engagement_engine.betrayal_cards_system.generate_inventory_display(user.id)
...
async def use_card_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for card_type, count in inventory.items():
        if count > 0:
            card_info = engagement_engine.betrayal_cards_system.get_card_info(CardType(card_type))
            if card_info:
                keyboard_buttons.append([
                    InlineKeyboardButton(
...
async def share_result_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("📊 View Sharing Stats", callback_data="sharing_stats")],
...
async def show_sharing_leaderboard_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Show sharing leaderboard."""
    leaderboard = engagement_engine.shareable_results_system.generate_sharing_leaderboard()
...
async def show_engagement_summary_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Show comprehensive engagement summary."""
    user = update.effective_user
    summary = engagement_engine.generate_engagement_summary(user.id)
    
    keyboard = InlineKeyboardMarkup([
        [InlineKeyboardButton("📊 Engagement Stats", callback_data="engagement_stats")],
...
async def handle_engagement_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        if data == "change_title":
            ...
            user = update.effective_user
            success, message = engagement_engine.flash_games_system.join_flash_event(user.id, event_id)
