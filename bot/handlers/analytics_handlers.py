...
async def show_session_leaderboard(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await update.message.reply_text("❌ Invalid session ID. Please provide a number.")
        return
    ...
    for entry in leaderboard:
        ...
        outcome_emoji = "✅" if entry['outcome'] == "win" else "❌" if entry['outcome'] == "lose" else "🤷"
...
async def show_recent_games(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Get recent sessions
    recent_sessions = db.query(GameSession).order_by(
...
async def show_analytics_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("🏆 Global Leaderboard", callback_data="analytics_global_lb")],
        [InlineKeyboardButton("🎮 Recent Games", callback_data="analytics_recent_games")],
        [InlineKeyboardButton("📈 Chat Stats", callback_data="analytics_chat_stats")]
    ])
