...
async def create_topic_game_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Handler for creating a new topic-based game."""
    await topic_handler.create_game_topic(update, context)


async def join_topic_game_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
...
async def start_topic_game_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
...
async def join_queue_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if topic_handler.topic_manager.add_to_queue(user_id):
        await query.answer("✅ You have joined the queue! You'll be notified when a game is available.")
    else:
        await query.answer("⚠️ You are already in the queue.")


async def show_available_games_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    all_full = all(metadata['player_count'] >= metadata['max_players'] for _, metadata in available_topics)
    keyboard_buttons = []
    ...
    if all_full:
        keyboard_buttons.append([InlineKeyboardButton("⏳ Join Queue", callback_data="join_queue")])
...
async def topic_message_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
...
async def rules_topic_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
...
async def close_topic_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
...
async def leaderboard_topic_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        leaderboard = sorted(players.items(), key=lambda x: x[1].get('xp', 0), reverse=True)
