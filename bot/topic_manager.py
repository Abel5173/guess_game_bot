...
class TopicManager:
    ...
    def create_game_session(self, topic_id: int, topic_name: str, creator_id: int, chat_id: int) -> ImpostorGame:
    ...
    def get_available_topics(self, chat_id: int) -> List[Tuple[int, dict]]:
        ...
        for topic_id, metadata in self.topic_metadata.items():
            if (metadata['status'] == 'waiting' and 
    ...
    def recover_active_sessions(self):
        ...
        for db_session in active_sessions:
            try:
                ...
                logger.info(f"Recovered session {db_session.id} for topic {db_session.topic_id}")
...
class TopicGameHandler:
    ...
    async def notify_queue(self, context: ContextTypes.DEFAULT_TYPE, topic_id: int, metadata: dict):
    ...
    async def create_game_topic(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> Optional[int]:
        """
        ...
        try:
            ...
            game = self.topic_manager.create_game_session(topic_id, topic_name, user.id, chat.id)
            ...
            join_keyboard = InlineKeyboardMarkup([
                [InlineKeyboardButton("🎮 Join Game", callback_data=f"join_topic_{topic_id}")],
                [InlineKeyboardButton("📋 Rules", callback_data=f"rules_topic_{topic_id}")],
                [InlineKeyboardButton("🏆 Leaderboard", callback_data=f"leaderboard_topic_{topic_id}")],
                [InlineKeyboardButton("❌ Close Topic", callback_data=f"close_topic_{topic_id}")]
            ])
            ...
            # Notify queued users
            await self.notify_queue(context, topic_id, self.topic_manager.topic_metadata[topic_id])
            return topic_id
    ...
    async def join_topic_game(self, update: Update, context: ContextTypes.DEFAULT_TYPE, topic_id: int):
    ...
    async def start_topic_game(self, update: Update, context: ContextTypes.DEFAULT_TYPE, topic_id: int):
        ...
        if user.id != metadata['creator_id']:
            await update.callback_query.answer("❌ Only the game creator can start the game!")
            return
    ...
    async def _update_topic_status(self, context: ContextTypes.DEFAULT_TYPE, topic_id: int, game: ImpostorGame, metadata: dict):
        ...
        try:
            ...
            if not chat_id and hasattr(context, 'bot') and hasattr(context.bot, 'chat_id'):
    ...
    async def handle_topic_message(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    async def show_available_games(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        keyboard_buttons.append([InlineKeyboardButton("🎮 Create New Game", callback_data="create_topic_game")])
