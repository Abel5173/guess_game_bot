...
class WhisperSystem:
    ...
    def send_whisper(
        ...
        if not self._validate_players_in_session(
        ...
        if custom_message:
            ...
            message = self._generate_whisper_message(
                whisper_type, from_player_id, to_player_id
            )
        ...
        # Log to database
        self._log_whisper_to_db(
            session_id, from_player_id, to_player_id, whisper_type, message
        )

        logger.info(
            f"Whisper sent: {from_player_id} -> {to_player_id} ({whisper_type.value})"
        )
    ...
    def mark_whisper_read(self, session_id: int, player_id: int, whisper_index: int):
        ...
        if (
            session_id in self.active_whispers
            and player_id in self.active_whispers[session_id]
            and whisper_index < len(self.active_whispers[session_id][player_id])
        ):
    ...
    def _validate_players_in_session(
    ...
    def _generate_whisper_message(
        self, whisper_type: WhisperType, from_player_id: int, to_player_id: int
    ) -> str:
    ...
    def _get_other_players_in_session(
