...
class MissionSystem:
    ...
    def __init__(self):
        ...
        self.mission_templates = {
            ...
            "daily_complete_5_tasks": Mission(
                ...
                MissionCategory.GAMEPLAY,
                {"tasks_completed": 5},
                {"xp": 30, "badge": "task_master"},
                1,
            ),
            "daily_vote_correctly": Mission(
                "daily_vote_correctly",
                "Sharp Eye",
                "Vote correctly 2 times today",
                MissionType.DAILY,
                MissionCategory.GAMEPLAY,
                {"correct_votes": 2},
                {"xp": 20, "badge": "voter"},
            ...
            "weekly_play_15_games": Mission(
                "weekly_play_15_games",
                "Dedicated Player",
                "Play 15 games this week",
                MissionType.WEEKLY,
                MissionCategory.GAMEPLAY,
                {"games_played": 15},
                {"xp": 100, "badge": "dedicated"},
                7,
            ),
            "weekly_win_5_games": Mission(
                ...
                MissionCategory.GAMEPLAY,
                {"games_won": 5},
                {"xp": 150, "badge": "warrior"},
                7,
            ),
            "weekly_complete_25_tasks": Mission(
                "weekly_complete_25_tasks",
                "Task Champion",
                "Complete 25 tasks this week",
                MissionType.WEEKLY,
                MissionCategory.GAMEPLAY,
                {"tasks_completed": 25},
                {"xp": 120, "badge": "champion"},
                7,
            ),
            "weekly_win_as_impostor": Mission(
                "weekly_win_as_impostor",
                "Deception Master",
                "Win 3 times as impostor this week",
                MissionType.WEEKLY,
                MissionCategory.GAMEPLAY,
                {"impostor_wins": 3},
                {"xp": 200, "badge": "deceiver"},
                7,
            ),
            # Seasonal missions
            "seasonal_halloween": Mission(
                "seasonal_halloween",
                "🎃 Spooky Season",
                "Win 10 games during October",
                MissionType.SEASONAL,
                MissionCategory.ACHIEVEMENT,
                {"seasonal_wins": 10, "season": "october"},
                {"xp": 500, "title": "🎃 Ghost of October"},
                31,
            ),
            "seasonal_winter": Mission(
                "seasonal_winter",
                "❄️ Winter Wonderland",
                "Win 10 games during December",
                MissionType.SEASONAL,
                MissionCategory.ACHIEVEMENT,
                {"seasonal_wins": 10, "season": "december"},
                {"xp": 500, "title": "❄️ Winter Guardian"},
                31,
            ),
        }
    ...
    def assign_daily_missions(self, player_id: int) -> List[Mission]:
        ...
        daily_templates = [
            m
            for m in self.mission_templates.values()
        ...
        logger.info(
            f"Assigned {len(assigned_missions)} daily missions to player {player_id}"
        )
    ...
    def assign_weekly_missions(self, player_id: int) -> List[Mission]:
        ...
        selected_missions = random.sample(
            weekly_templates, min(2, len(weekly_templates))
        )
        ...
        logger.info(
            f"Assigned {len(assigned_missions)} weekly missions to player {player_id}"
        )
        return assigned_missions

    def update_mission_progress(
        self, player_id: int, action: str, value: int = 1
    ) -> List[Dict]:
    ...
    def claim_mission_rewards(
        self, player_id: int, mission_id: str
    ) -> Tuple[bool, str]:
        """Claim rewards for a completed mission."""
        if (
            player_id not in self.active_missions
            or mission_id not in self.active_missions[player_id]
        ):
    ...
    def _cleanup_expired_missions(self, player_id: int):
        ...
        if expired_missions:
            logger.info(
                f"Cleaned up {len(expired_missions)} expired missions for player {player_id}"
            )
