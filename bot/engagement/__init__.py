...
class EngagementEngine:
    ...
    def _check_trophy_unlocks(self, player_id: int, game_result: Dict) -> List[Dict]:
        ...
        if game_result.get("won", False) and game_result.get("first_win", False):
            trophy = self.basecamp_system.unlock_trophy(
                player_id, TrophyType.FIRST_WIN
            )
            if trophy:
                unlocks.append({
                    ...
                    if title_name != current_title:
                        ...
                        if self.status_system.assign_title(player_id, title_name):
                            ...
                            updates.append({
                                ...
                                "rarity": title_info.get("rarity", "common") if title_info else "common"
                            })
        finally:
            db.close()
    ...
    def get_engagement_stats(self, player_id: int) -> Dict:
        ...
        # Get sharing stats
        sharing_stats = self.shareable_results_system.get_player_sharing_stats(player_id)
