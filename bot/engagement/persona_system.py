...
class PersonaSystem:
    ...
    def assign_persona_challenge(
        self, session_id: int, player_id: int, persona_type: PersonaType = None
    ) -> Dict:
    ...
    def analyze_message(self, session_id: int, player_id: int, message: str) -> Dict:
        ...
        if persona_type == PersonaType.DETECTIVE:
            if any(
                ...
                for phrase in ["evidence", "analyze", "based on"]
            ):
                compliance_score += 1
            ...
            if any(
                phrase in message.lower()
                for phrase in ["suspicious", "don't trust", "everyone"]
            ):
                compliance_score += 1
            ...
            if any(
                phrase in message.lower() for phrase in ["believe", "trust", "together"]
            ):
                compliance_score += 1
        ...
        return {
            ...
            "feedback": (
                " ".join(feedback) if feedback else "Keep working on your persona!"
            ),
    ...
    def get_player_challenge(self, session_id: int, player_id: int) -> Optional[Dict]:
        ...
        if (
            session_id in self.active_challenges
            and player_id in self.active_challenges[session_id]
        ):
            return self.active_challenges[session_id][player_id]
        return None
    ...
    def get_challenge_progress(self, session_id: int, player_id: int) -> Dict:
        ...
        compliance_rate = (
            (persona_messages / total_messages * 100) if total_messages > 0 else 0
        )
    ...
    def complete_challenge(self, session_id: int, player_id: int) -> Tuple[bool, int]:
        ...
        progress = self.challenge_progress[session_id][player_id]
        compliance_rate = (
            (progress["persona_messages"] / progress["total_messages"] * 100)
