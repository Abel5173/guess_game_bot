...
class BetrayalCard:
    ...
    def __init__(self, card_type: CardType, name: str, description: str, 
                 rarity: str, effect: Dict, usage_limit: int = 1):
...
class BetrayalCardsSystem:
    ...
    def __init__(self):
        ...
        self.card_definitions = {
            CardType.FAKE_TASK: BetrayalCard(
                CardType.FAKE_TASK, "Fake Task", "Complete a fake task to earn XP",
                "common", {"xp_gain": 5, "fake_task": True}, 1
            ),
            CardType.DOUBLE_VOTE: BetrayalCard(
                CardType.DOUBLE_VOTE, "Double Vote", "Your vote counts twice this round",
                "rare", {"vote_multiplier": 2}, 1
            ),
            CardType.ANONYMOUS_VOTE: BetrayalCard(
                CardType.ANONYMOUS_VOTE, "Anonymous Vote", "Vote without revealing your choice",
                "epic", {"anonymous_vote": True}, 1
            ),
            CardType.REVEAL_ROLE: BetrayalCard(
                CardType.REVEAL_ROLE, "Role Reveal", "Reveal another player's role",
                "legendary", {"reveal_role": True}, 1
            ),
            CardType.SABOTAGE_VOTE: BetrayalCard(
                CardType.SABOTAGE_VOTE, "Vote Sabotage", "Cancel another player's vote",
                "epic", {"sabotage_vote": True}, 1
            ),
            CardType.IMMUNITY: BetrayalCard(
                CardType.IMMUNITY, "Immunity", "Cannot be voted out this round",
                "legendary", {"immunity": True}, 1
            ),
            CardType.WHISPER: BetrayalCard(
                CardType.WHISPER, "Whisper", "Send a secret message to another player",
                "rare", {"whisper": True}, 3
            ),
            CardType.DECOY: BetrayalCard(
                CardType.DECOY, "Decoy", "Create a fake player to confuse others",
                "epic", {"decoy": True}, 1
            )
        }
    ...
    def add_card_to_inventory(self, player_id: int, card_type: CardType, count: int = 1) -> bool:
    ...
    def use_card(self, player_id: int, card_type: CardType, game_id: str, 
        ...
        # Apply card effect
        effect_result = self._apply_card_effect(card, player_id, game_id, target_player_id)
    ...
    def _apply_card_effect(self, card: BetrayalCard, player_id: int, game_id: str, 
                          target_player_id: Optional[int] = None) -> str:
    ...
    def get_card_usage_stats(self, player_id: int) -> Dict:
        ...
        if stats["cards_by_type"]:
            stats["most_used_card"] = max(stats["cards_by_type"].items(), 
                                        key=lambda x: x[1])[0]
