...
class BasecampSystem:
    ...
    def __init__(self):
        ...
        self.trophy_definitions = {
            ...
            TrophyType.IMPOSTOR_MASTER: Trophy(
                TrophyType.IMPOSTOR_MASTER,
                "Impostor Master",
                "Win 10 times as impostor",
                "🎭",
                "epic",
            ),
            TrophyType.VOTING_ACCURATE: Trophy(
                TrophyType.VOTING_ACCURATE,
                "Sharp Eye",
                "Vote correctly 20 times",
                "👁️",
                "rare",
            ),
                ...
                try:
                    ...
                    for trophy_info in trophy_data:
                        trophy = Trophy(
                            ...
                            trophy_info["rarity"],
                            (
                                datetime.fromisoformat(trophy_info["unlocked_at"])
                                if trophy_info.get("unlocked_at")
                                else None
                            ),
                        )
                ...
                "stats": {
                    ...
                    "rare_trophies": len(
                        [
                            t
                            for t in trophies
                            if t.rarity in ["rare", "epic", "legendary"]
                        ]
                    ),
                    "win_rate": (
    ...
    def unlock_trophy(
        self, player_id: int, trophy_type: TrophyType, custom_name: Optional[str] = None
    ) -> Optional[Trophy]:
            ...
            for rarity in ["legendary", "epic", "rare", "common"]:
                if rarity in rarity_groups:
                    ...
                    if len(rarity_groups[rarity]) > 3:
                        trophy_display += (
                            f"  ... and {len(rarity_groups[rarity]) - 3} more\n"
                        )
        else:
