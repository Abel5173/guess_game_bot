...
from bot.database.models import (
    GameSession, PlayerGameLink, VoteHistory, 
    DiscussionLog, TaskLog, JoinQueue, Player
)
import logging
...
class GameSessionManager:
    ...
    def create_session(self, topic_id: int, chat_id: int, creator_id: int, 
                      topic_name: str, game_type: str = "impostor") -> GameSession:
    ...
    def get_session_by_topic(self, topic_id: int) -> Optional[GameSession]:
        ...
        return self.db.query(GameSession).filter(GameSession.topic_id == topic_id).first()
    ...
    def update_session_status(self, topic_id: int, status: str, 
    ...
    def add_player_to_session(self, session_id: int, player_id: int, 
                            role: str = "crewmate") -> PlayerGameLink:
        """Add a player to a game session."""
        # Check if player is already in session
        existing = self.db.query(PlayerGameLink).filter(
            and_(
    ...
    def remove_player_from_session(self, session_id: int, player_id: int) -> bool:
        ...
        link = self.db.query(PlayerGameLink).filter(
            and_(
                PlayerGameLink.session_id == session_id,
                PlayerGameLink.player_id == player_id,
                PlayerGameLink.left_at.is_(None)
    ...
    def get_session_players(self, session_id: int) -> List[PlayerGameLink]:
        ...
        return self.db.query(PlayerGameLink).filter(
            and_(
                PlayerGameLink.session_id == session_id,
                PlayerGameLink.left_at.is_(None)
            )
    ...
    def log_vote(self, session_id: int, voter_id: int, target_id: int, 
                round_number: int = 1, vote_type: str = "eject") -> VoteHistory:
    ...
    def log_discussion(self, session_id: int, player_id: int, message: str, 
                      phase: str = "discussion") -> DiscussionLog:
    ...
    def log_task_completion(self, session_id: int, player_id: int, 
                          task_type: str, task_description: str, 
                          xp_earned: int = 0) -> TaskLog:
...
class JoinQueueManager:
    ...
    def add_to_queue(self, player_id: int, chat_id: int) -> bool:
        ...
        existing = self.db.query(JoinQueue).filter(
            and_(
    ...
    def cleanup_old_queue(self, max_age_hours: int = 6) -> int:
        ...
        cutoff_time = datetime.now() - timedelta(hours=max_age_hours)
        old_entries = self.db.query(JoinQueue).filter(
            JoinQueue.joined_at < cutoff_time
        ).all()
...
class AnalyticsManager:
    ...
    def get_player_stats(self, player_id: int) -> Dict[str, Any]:
        ...
        games_played = self.db.query(PlayerGameLink).filter(
            PlayerGameLink.player_id == player_id
        ).count()
        
        games_won = self.db.query(PlayerGameLink).filter(
            and_(
                PlayerGameLink.player_id == player_id,
        ...
        total_xp_earned = self.db.query(PlayerGameLink).filter(
            PlayerGameLink.player_id == player_id
        ).with_entities(func.sum(PlayerGameLink.xp_earned)).scalar() or 0
    ...
    def get_session_leaderboard(self, session_id: int) -> List[Dict[str, Any]]:
        ...
        players = self.db.query(PlayerGameLink).filter(
            PlayerGameLink.session_id == session_id
        ).join(Player).order_by(desc(PlayerGameLink.xp_earned)).all()
    ...
    def get_chat_stats(self, chat_id: int, days: int = 7) -> Dict[str, Any]:
        ...
        active_sessions = len([s for s in sessions if s.status in ["waiting", "active"]])
        ...
        return {
            ...
            "completion_rate": finished_sessions / total_sessions if total_sessions > 0 else 0
        } 