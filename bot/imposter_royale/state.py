...
class GameState:
    ...
    async def _get_or_create_session(self):
        ...
        session = await loop.run_in_executor(
            None,
            lambda: self.db.query(GameSession).filter_by(chat_id=self.chat_id, game_type="imposter_royale").first()
        )
        if not session:
            session = GameSession(chat_id=self.chat_id, game_type="imposter_royale", status="waiting")
            self.db.add(session)
    ...
    async def add_player(self, player_obj):
        ...
        player_in_db = await loop.run_in_executor(
            None,
            lambda: self.db.query(Player).filter_by(id=player_obj.user_id).first()
        )
        if not player_in_db:
        ...
        link = PlayerGameLink(player_id=player_in_db.id, session_id=self.game_session.id, role=player_obj.role)
        self.db.add(link)
        await loop.run_in_executor(None, self.db.commit)
    ...
    async def get_all_players(self):
        ...
        return await loop.run_in_executor(
            None,
            lambda: self.db.query(Player).join(PlayerGameLink).filter(PlayerGameLink.session_id == self.game_session.id).all()
        )
    ...
    async def update_player_status(self, user_id, status):
        ...
        link = await loop.run_in_executor(
            None,
            lambda: self.db.query(PlayerGameLink).filter_by(player_id=user_id, session_id=self.game_session.id).first()
        )
        if link:
            link.outcome = status
            await loop.run_in_executor(None, self.db.commit)
