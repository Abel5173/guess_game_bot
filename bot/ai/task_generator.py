...
class AITaskGenerator:
    ...
    def __init__(self):
        ...
        self.locations = [
            "Engineering Bay", "Medical Wing", "Navigation Deck", "Cargo Hold",
            "Communications Hub", "Life Support", "Power Grid", "Shield Generator",
            "Weapons Bay", "Science Lab", "Crew Quarters", "Command Bridge"
        ]
        
        self.systems = [
            "life support", "navigation", "communications", "power distribution",
            "shields", "weapons", "sensors", "thrusters", "artificial gravity",
            "environmental controls", "security", "medical systems"
        ]
        
        self.equipment = [
            "oxygen scrubbers", "fusion reactors", "plasma conduits", "quantum computers",
            "holographic projectors", "tractor beams", "warp drives", "phaser arrays",
            "transporter pads", "replicators", "holodecks", "deflector dishes"
        ]
        
        self.supplies = [
            "medical supplies", "food rations", "spare parts", "energy cells",
            "oxygen tanks", "water containers", "repair kits", "emergency gear"
        ]
    
    async def generate_task(self, session_id: int, player_id: int, role: str, 
    ...
    async def generate_batch_tasks(self, session_id: int, player_ids: List[int], 
                                 roles: Dict[int, str]) -> Dict[int, Dict]:
    ...
    def complete_task(self, session_id: int, player_id: int, task_id: str) -> Tuple[bool, int]:
        ...
        for t in self.task_history[session_id]:
            if t["id"] == task_id and t["player_id"] == player_id and not t["completed"]:
                task = t
                break
        ...
        # Update progress
        if (session_id in self.player_task_progress and 
            player_id in self.player_task_progress[session_id]):
            self.player_task_progress[session_id][player_id]["tasks_completed"] += 1
            self.player_task_progress[session_id][player_id]["total_xp_earned"] += xp_reward
    ...
    def get_player_tasks(self, session_id: int, player_id: int) -> List[Dict]:
        ...
        return [task for task in self.task_history[session_id] 
    ...
    def get_task_progress(self, session_id: int, player_id: int) -> Dict:
        ...
        if (session_id not in self.player_task_progress or 
            player_id not in self.player_task_progress[session_id]):
            return {
                "tasks_assigned": 0,
                "tasks_completed": 0,
                "total_xp_earned": 0,
